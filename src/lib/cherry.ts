...
/** Whether the proposal can be applied at all. */
export function isBlocked(p: CherryProposal): boolean {
  return p.blocked_by !== "nothing";
}

/** Questions that must be answered before anything can be written. */
export function blockingQuestions(p: CherryProposal): CherryQuestion[] {
  return p.questions.filter((q) => q.blocking && q.answered_value === undefined && !q.skipped);
}

/** Questions worth offering, which the user may skip. */
export function optionalQuestions(p: CherryProposal): CherryQuestion[] {
  return p.questions.filter((q) => !q.blocking && q.answered_value === undefined && !q.skipped);
}

/** Actions that are checked by default. Destructive ones never are. */
export function defaultConfirmedIds(p: CherryProposal): string[] {
  return p.actions.filter((a) => a.ready && a.severity !== "destructive").map((a) => a.id);
}

export function operationVerb(op: CherryOperation): string {
  return op === "insert" ? "Create" : op === "update" ? "Update" : "Delete";
}
